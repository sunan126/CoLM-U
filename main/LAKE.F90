...
! PUBLIC MEMBER FUNCTIONS:
  public :: newsnow_lake
  public :: laketem
  public :: laketem_snicar
  public :: snowwater_lake
  public :: snowwater_lake_snicar
...
!      end if
      ...
      do j = 1, nl_lake
         ...
         zout = z_lake(j) + 0.5*dz_lake(j)
         rsfin  = exp( -eta*max(  zin-za(idlak),0. ) )  ! the radiation within surface layer (z<za)
         rsfout = exp( -eta*max( zout-za(idlak),0. ) )  ! is considered fixed at (1-beta)*sabg_lyr(1)
                                                        ! i.e, max(z-za, 0)
         ! Let rsfout for bottom layer go into soil.
         ! This looks like it should be robust even for pathological cases,
         ! like lakes thinner than za(idlak).
      ...
      if(abs(sum(phi(1:nl_lake)) + phix(nl_lake+1) - sabg_lyr(1)*(1.-betaprime)) > 1.0e-6)then
         print*, 'Error in solar absorption in lake layers', &
                 'sum(phi(1:nl_lake)) = ', sum(phi(1:nl_lake)), &
                 'phix(nl_lake+1) = ', phix(nl_lake+1), &
                 'sabg_lyr(1) = ', sabg_lyr(1)
         call abort
      endif
! January 12, 2023
...
! January 12, 2023
   ...
   !  dhsdT = - cgrnd - 4.*emg*stefnc*tx_bef(lb)**3
      dhsdT = 0.0
