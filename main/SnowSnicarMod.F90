...
! save
  ...
  ! (arrays declared here, but are set in iniTimeConst)
  !
  real(r8) :: snowage_tau   (idx_rhos_max,idx_Tgrd_max,idx_T_max)  ! snow aging parameter retrieved from lookup table [hour]
  real(r8) :: snowage_kappa (idx_rhos_max,idx_Tgrd_max,idx_T_max)  ! snow aging parameter retrieved from lookup table [unitless]
  real(r8) :: snowage_drdt0 (idx_rhos_max,idx_Tgrd_max,idx_T_max)  ! snow aging parameter retrieved from lookup table [um hr-1]
...
contains
  ...
  subroutine SNICAR_RT (flg_snw_ice, flg_slr_in, &
    ...
    real(r8) , intent(in)  :: coszen         ( begc:endc )  ! cosine of solar zenith angle for next time step (col) [unitless]

    integer  , intent(in)  :: snl            ( begc:endc )  !  negative number of snow layers (col) [nbr]
    real(r8) , intent(in)  :: h2osno         ( begc:endc )  !  snow liquid water equivalent (col) [kg/m2]
    ...
    real(r8) , intent(in)  :: h2osno_ice     ( begc:endc , -nlevsno+1:0 )   ! ice content (col,lyr) [kg/m2]
    integer  , intent(in)  :: snw_rds        ( begc:endc , -nlevsno+1:0 )   ! snow effective radius (col,lyr) [microns, m^-6]
    real(r8) , intent(in)  :: mss_cnc_aer_in ( begc:endc , -nlevsno+1:0 , 1:sno_nbr_aer ) ! mass concentration of all aerosol species (col,lyr,aer) [kg/kg]
    real(r8) , intent(in)  :: albsfc         ( begc:endc , 1:numrad )       ! albedo of surface underlying snow (col,bnd) [frc]
    real(r8) , intent(out) :: albout         ( begc:endc , 1:numrad )       ! snow albedo, averaged into 2 bands (=0 if no sun or no snow) (col,bnd) [frc]
    real(r8) , intent(out) :: flx_abs        ( begc:endc , -nlevsno+1:1 , 1:numrad ) ! absorbed flux in each layer per unit flux incident (col, lyr, bnd)
    !
    ! !LOCAL VARIABLES:
    !
...
!H. Wang
  ...
  !-----------------------------------------------------------------------
    ...
    integer  , intent(in)   ::  snl              ( begc:endc )  !  negative number of snow layers (col) [nbr]
    real(r8) , intent(in)   ::  dz               ( begc:endc,-nlevsno+1:1 ) !  layer thickness (col,lyr) [m]

    real(r8) , intent(in)   ::  qflx_snow_grnd   ( begc:endc )  !  snow on ground after interception (col) [kg m-2 s-1]
    real(r8) , intent(in)   ::  qflx_snwcp_ice   ( begc:endc )  !  excess precipitation due to snow capping [kg m-2 s-1]
    real(r8) , intent(in)   ::  qflx_snofrz_lyr  ( begc:endc,-nlevsno+1:0 ) !  snow freezing rate (col,lyr) [kg m-2 s-1]

    logical  , intent(in)   ::  do_capsnow       ( begc:endc )  !  true => do snow capping
    real(r8) , intent(in)   ::  frac_sno         ( begc:endc )  !  fraction of ground covered by snow (0 to 1)
    real(r8) , intent(in)   ::  h2osno           ( begc:endc )  !  snow water (col) [mm H2O]
    real(r8) , intent(in)   ::  h2osno_liq       ( begc:endc,-nlevsno+1:0 ) !  liquid water content (col,lyr) [kg m-2]
    real(r8) , intent(in)   ::  h2osno_ice       ( begc:endc,-nlevsno+1:0 ) !  ice content (col,lyr) [kg m-2]

    real(r8) , intent(in)   ::  t_soisno         ( begc:endc,-nlevsno+1:1 ) !  soil and snow temperature (col,lyr) [K]
    real(r8) , intent(in)   ::  t_grnd           ( begc:endc )  !  ground temperature (col) [K]

    real(r8) , intent(inout)  ::  snw_rds        ( begc:endc,-nlevsno+1:0 ) !  effective grain radius (col,lyr) [microns, m-6]
  ...
  !-----------------------------------------------------------------------
    ...
    real(r8) :: dTdz(begc:endc,-nlevsno:0)  !  snow temperature gradient (col,lyr) [K m-1]
    ...
    real(r8) :: snot_top    ( begc:endc )  !  temperature in top snow layer (col) [K]
    real(r8) :: dTdz_top    ( begc:endc )  !  temperature gradient in top layer (col) [K m-1]
    real(r8) :: snw_rds_top ( begc:endc )  !  effective grain radius, top layer (col) [microns, m-6]
    real(r8) :: sno_liq_top ( begc:endc )  !  liquid water fraction (mass) in top snow layer (col) [frc]
 ...
 !       )
      ...
      do c_idx = begc, endc
         ...
         if (snl(c_idx) < 0 .and. h2osno(c_idx) > 0._r8) then
         ...
         do i = snl_top, snl_btm, 1
            ...
            !**********  1. DRY SNOW AGING  ***********
            !
            h2osno_lyr = h2osno_liq(c_idx,i) + h2osno_ice(c_idx,i)
            ...
            ! snow density
            rhos = (h2osno_liq(c_idx,i)+h2osno_ice(c_idx,i)) / cdz(i)
            ...
            if ( abs(dr_fresh) < 1.0e-8_r8 ) then
               ...
               write(iulog,*) "dr_fresh = ", dr_fresh, snw_rds(c_idx,i), snw_rds_min
               call abort
            end if
            ...
            frc_liq = min(0.1_r8, (h2osno_liq(c_idx,i) / (h2osno_liq(c_idx,i)+h2osno_ice(c_idx,i))))
            ...
            if (do_capsnow(c_idx) .and. .not. use_extrasnowlayers) then
               ...
               newsnow = max(0._r8, (qflx_snow_grnd(c_idx)*dtime))
            endif
            ...
            if (i == snl_top) then
               ...
               sno_liq_top(c_idx) = h2osno_liq(c_idx,i) / (h2osno_liq(c_idx,i)+h2osno_ice(c_idx,i))
            endif
     
         enddo
         endif   ! endif (snl(c_idx) < 0 )
  ...
  !-----------------------------------------------------------------------
	...
	write(iulog,*) "snicar_atm_type = ", snicar_atm_type
      ...
      call nccheck( nf90_open(trim(fsnowoptics), NF90_NOWRITE, ncid) )

      write(iulog,*) subname,trim(fsnowoptics)

      ! direct-beam snow Mie parameters:
      call nccheck( nf90_inq_varid(ncid, 'ss_alb_ice_drc', varid) )
      call nccheck( nf90_get_var(ncid, varid, ss_alb_snw_drc) )

      call nccheck( nf90_inq_varid(ncid, 'asm_prm_ice_drc', varid) )
      call nccheck( nf90_get_var(ncid, varid, asm_prm_snw_drc) )

      call nccheck( nf90_inq_varid(ncid, 'ext_cff_mss_ice_drc', varid) )
      call nccheck( nf90_get_var(ncid, varid, ext_cff_mss_snw_drc) )
      !
      ! diffuse snow Mie parameters
      !
      call nccheck( nf90_inq_varid(ncid, 'ss_alb_ice_dfs', varid) )
      call nccheck( nf90_get_var(ncid, varid, ss_alb_snw_dfs) )

      call nccheck( nf90_inq_varid(ncid, 'asm_prm_ice_dfs', varid) )
      call nccheck( nf90_get_var(ncid, varid, asm_prm_snw_dfs) )

      call nccheck( nf90_inq_varid(ncid, 'ext_cff_mss_ice_dfs', varid) )
      call nccheck( nf90_get_var(ncid, varid, ext_cff_mss_snw_dfs) )
	  
      !!! Direct and diffuse flux under different atmospheric conditions
      ! Direct-beam incident spectral flux: 
      call nccheck( nf90_inq_varid(ncid, 'flx_wgt_dir', varid) )
      call nccheck( nf90_get_var(ncid, varid, flx_wgt_dir) )
      if ((atm_type_index > 0) .and. (.not. readvar)) call abort

      ! Diffuse incident spectral flux:
      call nccheck( nf90_inq_varid(ncid, 'flx_wgt_dif', varid) )
      call nccheck( nf90_get_var(ncid, varid, flx_wgt_dif) )
      if ((atm_type_index > 0) .and. (.not. readvar)) call abort
     ...
     !
     call nccheck( nf90_inq_varid(ncid, 'ss_alb_bc_mam', varid) )
     call nccheck( nf90_get_var(ncid, varid, ss_alb_bc1) )
     if (.not. readvar) call abort

     call nccheck( nf90_inq_varid(ncid, 'asm_prm_bc_mam', varid) )
     call nccheck( nf90_get_var(ncid, varid, asm_prm_bc1) )
     if (.not. readvar) call abort

     call nccheck( nf90_inq_varid(ncid, 'ext_cff_mss_bc_mam', varid) )
     call nccheck( nf90_get_var(ncid, varid, ext_cff_mss_bc1) )
     if (.not. readvar) call abort
     ...
     !
     call nccheck( nf90_inq_varid(ncid, 'ss_alb_bc_mam', varid) )
     call nccheck( nf90_get_var(ncid, varid, ss_alb_bc2) )
     if (.not. readvar) call abort

     call nccheck( nf90_inq_varid(ncid, 'asm_prm_bc_mam', varid) )
     call nccheck( nf90_get_var(ncid, varid, asm_prm_bc2) )
     if (.not. readvar) call abort

     call nccheck( nf90_inq_varid(ncid, 'ext_cff_mss_bc_mam', varid) )
     call nccheck( nf90_get_var(ncid, varid, ext_cff_mss_bc2) )
     if (.not. readvar) call abort
     !
     ! size-dependent BC absorption enhancement factors for within-ice BC
     call nccheck( nf90_inq_varid(ncid, 'bcint_enh_mam', varid) )
     call nccheck( nf90_get_var(ncid, varid, bcenh) )
     if (.not. readvar) call abort
     !
#else
     ! bulk aerosol treatment
     ! BC species 1 Mie parameters
     call nccheck( nf90_inq_varid(ncid, 'ss_alb_bcphil', varid) )
     call nccheck( nf90_get_var(ncid, varid, ss_alb_bc1) )

     call nccheck( nf90_inq_varid(ncid, 'asm_prm_bcphil', varid) )
     call nccheck( nf90_get_var(ncid, varid, asm_prm_bc1) )

     call nccheck( nf90_inq_varid(ncid, 'ext_cff_mss_bcphil', varid) )
     call nccheck( nf90_get_var(ncid, varid, ext_cff_mss_bc1) )
     ! 
     ! BC species 2 Mie parameters
     !
     call nccheck( nf90_inq_varid(ncid, 'ss_alb_bcphob', varid) )
     call nccheck( nf90_get_var(ncid, varid, ss_alb_bc2) )

     call nccheck( nf90_inq_varid(ncid, 'asm_prm_bcphob', varid) )
     call nccheck( nf90_get_var(ncid, varid, asm_prm_bc2) )

     call nccheck( nf90_inq_varid(ncid, 'ext_cff_mss_bcphob', varid) )
     call nccheck( nf90_get_var(ncid, varid, ext_cff_mss_bc2) )
     !
#endif
     !
     ! OC species 1 Mie parameters
     call nccheck( nf90_inq_varid(ncid, 'ss_alb_ocphil', varid) )
     call nccheck( nf90_get_var(ncid, varid, ss_alb_oc1) )

     call nccheck( nf90_inq_varid(ncid, 'asm_prm_ocphil', varid) )
     call nccheck( nf90_get_var(ncid, varid, asm_prm_oc1) )

     call nccheck( nf90_inq_varid(ncid, 'ext_cff_mss_ocphil', varid) )
     call nccheck( nf90_get_var(ncid, varid, ext_cff_mss_oc1) )
     !
     ! OC species 2 Mie parameters
     call nccheck( nf90_inq_varid(ncid, 'ss_alb_ocphob', varid) )
     call nccheck( nf90_get_var(ncid, varid, ss_alb_oc2) )

     call nccheck( nf90_inq_varid(ncid, 'asm_prm_ocphob', varid) )
     call nccheck( nf90_get_var(ncid, varid, asm_prm_oc2) )

     call nccheck( nf90_inq_varid(ncid, 'ext_cff_mss_ocphob', varid) )
     call nccheck( nf90_get_var(ncid, varid, ext_cff_mss_oc2) )
     !
     ! dust species 1 Mie parameters
     call nccheck( nf90_inq_varid(ncid, 'ss_alb_dust01', varid) )
     call nccheck( nf90_get_var(ncid, varid, ss_alb_dst1) )

     call nccheck( nf90_inq_varid(ncid, 'asm_prm_dust01', varid) )
     call nccheck( nf90_get_var(ncid, varid, asm_prm_dst1) )

     call nccheck( nf90_inq_varid(ncid, 'ext_cff_mss_dust01', varid) )
     call nccheck( nf90_get_var(ncid, varid, ext_cff_mss_dst1) )
     !
     ! dust species 2 Mie parameters
     call nccheck( nf90_inq_varid(ncid, 'ss_alb_dust02', varid) )
     call nccheck( nf90_get_var(ncid, varid, ss_alb_dst2) )

     call nccheck( nf90_inq_varid(ncid, 'asm_prm_dust02', varid) )
     call nccheck( nf90_get_var(ncid, varid, asm_prm_dst2) )

     call nccheck( nf90_inq_varid(ncid, 'ext_cff_mss_dust02', varid) )
     call nccheck( nf90_get_var(ncid, varid, ext_cff_mss_dst2) )
     !
     ! dust species 3 Mie parameters
     call nccheck( nf90_inq_varid(ncid, 'ss_alb_dust03', varid) )
     call nccheck( nf90_get_var(ncid, varid, ss_alb_dst3) )

     call nccheck( nf90_inq_varid(ncid, 'asm_prm_dust03', varid) )
     call nccheck( nf90_get_var(ncid, varid, asm_prm_dst3) )

     call nccheck( nf90_inq_varid(ncid, 'ext_cff_mss_dust03', varid) )
     call nccheck( nf90_get_var(ncid, varid, ext_cff_mss_dst3) )
     !
     ! dust species 4 Mie parameters
     call nccheck( nf90_inq_varid(ncid, 'ss_alb_dust04', varid) )
     call nccheck( nf90_get_var(ncid, varid, ss_alb_dst4) )

     call nccheck( nf90_inq_varid(ncid, 'asm_prm_dust04', varid) )
     call nccheck( nf90_get_var(ncid, varid, asm_prm_dst4) )

     call nccheck( nf90_inq_varid(ncid, 'ext_cff_mss_dust04', varid) )
     call nccheck( nf90_get_var(ncid, varid, ext_cff_mss_dst4) )
     !
     !
  ...
  subroutine SnowAge_init( fsnowaging )
     ...
     !
     call nccheck( nf90_open(trim(fsnowaging), NF90_NOWRITE, ncid) )

     call nccheck( nf90_inq_varid(ncid, 'tau', varid) )
     call nccheck( nf90_get_var(ncid, varid, snowage_tau) )

     call nccheck( nf90_inq_varid(ncid, 'kappa', varid) )
     call nccheck( nf90_get_var(ncid, varid, snowage_kappa) )

     call nccheck( nf90_inq_varid(ncid, 'drdsdt0', varid) )
     call nccheck( nf90_get_var(ncid, varid, snowage_drdt0) )
