#include <define.h>
  ...
  ! !DESCRIPTION:
  ! make urban surface data for global, regional and point case
  ! !PURPOSE:
  ...
  !
  ! ORIGINAL:
  !   Wenzong Dong, Sep, 2022: initial version.
  ! -----------------------------------------------------------------------
  ! !USES:
    USE precision
    USE netcdf
    ...
    ! Global attribute of netCDF file
    ...
    INTEGER  :: i, j, k, io, jo, m, n, ii, jj, mm, cont, sumth, p, inx, im, jm, pop_i, year
    ...
    ! Urban raw data are available every five years, but surface data are every years,
    ! so need to convert lc_year to match the urban data.
    IF (mod(lc_year,5) == 0) THEN
       ! lc_year is 2000/2005/2010/2015/2020
       write(cyear,'(i4.4)') lc_year
    ELSE
       ! When lc_year is the rest of the year, take the last year that's divisible by 5,
       ! such as lc_year is 2004, then the year of urban data is 2000
    ...
    allocate( tk_wgt    (lon_points, lat_points, den_clss, ulev) )
    allocate( cv_wgt    (lon_points, lat_points, den_clss, ulev) )
...
#ifndef USE_LCZ
    ! look-up-table of NCAR urban properties
    ...
    DO reglat = sreglat, ereglat, -5
       DO reglon_ = sreglon, ereglon, 5
          ...
          lndname = TRIM(dir_rawdata)//'urb/RG_'//TRIM(adjustL(reg1))//'_'//&
                     TRIM(adjustL(reg2))//'_'//TRIM(adjustL(reg3))//'_'//TRIM(adjustL(reg4))//'.URB'//TRIM(cyear)//'.nc'
          ...
          ! get the raw data of htop/tree cover/water/roof fraction/building height/population density
          CALL nccheck( nf90_open(trim(lndname), nf90_nowrite, ncid) )
          ...
          CALL nccheck( nf90_inq_varid(ncid, "HT_ROOF"     , ht_rfvid   ) )
          CALL nccheck( nf90_inq_varid(ncid, "WTLUNIT_ROOF", wt_rfvid   ) )
          CALL nccheck( nf90_inq_varid(ncid, "POP"         , popvid     ) )
          ...
          CALL nccheck( nf90_get_var(ncid, popvid    , pop(:,:), &  
                        start=(/1,1,pop_i/), count=(/600,600,1/)) )

#ifdef USE_LCZ
          ! get the raw data of LCZ
          CALL nccheck( nf90_inq_varid(ncid, "LCZ"         , ur_clssvid ) )
          CALL nccheck( nf90_get_var  (ncid, ur_clssvid    , lcz        ) )
#else
          ! get the raw data of NCAR urban class and region ID
...
#ifdef USE_LCZ
          ! TODO: Merge the LSAI data sets to urb_5X5
          ! get the raw data of LCZ LAI
             ...
             DO j = sj, ej
                ...
                IF (lcz(j,i)>0 .and. lcz(j,i)<=10) THEN
                   IF (modur(jj,ii) > 0) THEN                           
                      ...
                      ! Area-weighted average of urban buliding height/fraction/population density/
                      ! tree cover/tree height/water cover and LSAI for LCZ
...
#else
          ! TODO: Merge the LSAI data sets to urb_5X5
          ! get the raw data of NCAR urban LSAI
          lndname = TRIM(dir_rawdata)//'lai_5x5/RG_'//TRIM(adjustL(reg1))//'_'//&
                     TRIM(adjustL(reg2))//'_'//TRIM(adjustL(reg3))//'_'//TRIM(adjustL(reg4))//'.UrLAI_v5'//TRIM(cyear)//'.nc'
          ...
          DO i = si, ei
             DO j = sj, ej
                ...
                IF (urden(j,i) > 0) THEN
                   ! Area-weighted average of urban tree cover/tree height/water cover and LSAI for each NCAR urban
                   inx = int(urden(j,i))
                   ...
                   ! Area-weighted average of urban buliding morphological and thermal parameter.
                   ! All parameters are look-up-table by uxid and inx, exclude building height and fraction
                   uxid             = urrgid(j,i)
                   ur_dc(jo,io,inx) = ur_dc(jo,io,inx) + harea(j,i)
                   
                   ! if grid data of ht_roof and wt_rf is nan, look-up-table to get value
                   IF (htrf(j,i) > 0) THEN
                   ...
                   ! Area-weighted average of urban population density
                   IF (pop(jm,im) > 0) THEN
                      pop_ur(jo,io,inx) = pop_ur(jo,io,inx) + pop(jm,im)*harea(j,i)
                   ENDIF
                ...
                IF (modur(j,i)>0 .and. urden(j,i)<=0) THEN
#endif
                   ! Some grid points of MODIS and NCAR are inconsistent (NCAR has no urban ID), 
                   ! so the these points are assigned by the 3(medium density), or can be defined by ueser 
                   IF (gl30_wt(j,i) > 0.) THEN
                   ...
                   uxid           = urrgid(j,i)
                   ! A particular point that NCAR is ocean and MODIS is urban,
                   ! so the region id is assigned by the nearest region id(30).
                   ...
                   ELSE
                      ht_rf(jo,io,3) = ht_rf(jo,io,3) + ncar_ht(3,uxid)*harea(j,i)
                   ENDIF
    ...
    ! assign LUCY region ID, nereast neighbor method
    DO i = 1, lat_points
       DO j=1, lon_points
       ...
       DO j = 1, lon_points
          DO k = 1, 10
             IF (ur_dc(j,i,k) > 0) THEN
                ...
                pct_tc  (j,i,k) = tc  (j,i,k) / ur_dc(j,i,k)
                ! calculate urban water cover/building height/building fraction/population density
                pct_urwt(j,i,k) = urwt(j,i,k) / ur_dc(j,i,k)
                ...
                IF (wgt_top(j,i,k) > 0.) THEN
                   ...
                   htop_ur (j,i,k) = htop(j,i,k) / wgt_top(j,i,k)
                ENDIF
                DO ii = 1, 12
                   IF (wgt_lai(j,i,k,ii) > 0.) THEN
                      ! calculate urban tree LAI
                      ur_lai (j,i,k,ii) = ur_lai(j,i,k,ii) / wgt_lai(j,i,k,ii)
                   ENDIF
                   IF (wgt_sai(j,i,k,ii) > 0.) THEN
                      ! calculate urban tree SAI
                      ur_sai (j,i,k,ii) = ur_sai(j,i,k,ii) / wgt_sai(j,i,k,ii)
                   ENDIF
                ENDDO
             ENDIF
          ENDDO

          ! grid tree height, only used when urban tree height is nan,
          ! but urban tree cover great than 0,
          ! because Simard data has nan value in many urban grids.
          IF (avg(j,i) > 0) THEN
             hgt(j,i) = hgt(j,i) / avg(j,i)
          ENDIF

          sumur = sum(ur_dc(j,i,1:10))
          IF (sumur > 0.) THEN
             ...
             ENDDO
          ENDIF
          ! Confirm that sum of every LCZs does not exceed 100%
    ...
    DO i = 1, lat_points
       DO j = 1, lon_points
          DO k =1, 10
             ! If the urban grid has vegetation cover but no tree height, the grid tree height will be assigned for urban tree
             IF (pct_tc(j,i,k) > 0 .and. htop_ur(j,i,k) == 0) THEN
                htop_ur(j,i,k) = hgt(j,i)
             ENDIF

             ! If there is no tree height data after the previous step
             ! Then the average height of all trees at that latitude is assigned for urban tree
             IF (pct_tc(j,i,k) > 0 .and. htop_ur(j,i,k) == 0) THEN
                DO p = 1, lon_points
                   IF (hgt(p,i) > 0) THEN
                      sumth = sumth + hgt(p,i)
    ...
    DO i = 1, lat_points
       DO j = 1, lon_points
                ...
                IF (ur_dc(j,i,k) > 0) THEN
                   ...
                   DO m = 1, 10
                      IF (tk_imrd(j,i,k,m) > 0.) THEN
                         tk_imrd(j,i,k,m) = tk_imrd(j,i,k,m) / tk_wgt(j,i,k,m)
                      ENDIF
                      IF (cv_imrd(j,i,k,m) > 0.) THEN
                         cv_imrd(j,i,k,m) = cv_imrd(j,i,k,m) / cv_wgt(j,i,k,m)
                      ENDIF
                   ENDDO
          ...
          ! Confirm that sum of every NCAR urban does not exceed 100%
